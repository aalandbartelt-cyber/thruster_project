...
def main():
    ...
    for file_idx, (_, row) in enumerate(df_normal.iterrows()):
        if file_idx % 100 == 0:
            print(f'  [{file_idx}/{len(df_normal)}] evaluating...', end='\r', flush=True)
    ...
    print(f'  [{len(df_normal)}/{len(df_normal)}] evaluating... done')
    ...
    for _, row in df_normal.iterrows():
        if iface_done % 200 == 0:
            print(f'  [{iface_done}/{n_iface}] generating interface files...', end='\r', flush=True)
        fpath = os.path.join(test_dir, row['filename'])
        try:
            df = pd.read_csv(fpath)
        except Exception:
